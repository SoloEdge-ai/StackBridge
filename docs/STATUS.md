...
| 范围 | 代码状态 | 验证状态 | 说明 |
|---|---|---|---|
| 仓库与设计资料 | implemented | verified | 本地 Git 与公开的 `SoloEdge-ai/StackBridge` 远端已建立，原始资料保留在 `docs/design/` |
...
| 连续 AI 对话 | implemented | verified-with-mock-deepseek | 每个对话冻结 ChatGPT/DeepSeek 提供方与模型，历史选择可恢复路由；ChatGPT 继续使用 Codex App Server，DeepSeek 直接使用 Responses API、结构化回答与建议、64 KiB 历史/终端上下文及可取消请求；DeepSeek 配置、脱敏、错误映射、旧 Codex 会话迁移和浏览器 UI 有自动化覆盖，真实 DeepSeek 密钥留给用户在应用内冒烟 |
...
| Windows 便携版 | implemented | verified-on-current-host | Electron 44 x64 单文件不内置 Codex；启动器核验系统 `codex --version`，失败时仅禁用 ChatGPT，窗口、Core、终端和 DeepSeek 仍启动；DeepSeek API Key 由 Electron `safeStorage` 注入的系统加密器保护，Core 与 SQLite 不保存明文 |
...
- ChatGPT 登录、凭据持久化和真实回答已通过随包 App Server；DeepSeek 已通过本地模拟 Responses API，真实密钥不进入仓库或 CI，仍需用户在应用内完成真实服务冒烟。
